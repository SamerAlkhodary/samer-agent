...
lint:
	uv run ruff check samer_agent/ --config .ruff.toml

lint-fix:
	uv run ruff check samer_agent/ --fix --config .ruff.toml
test-unit:
	uv run -m pytest --cov=samer_agent -vv tests/unit --cov-report=lcov:coverage/unit.info

test-component:
	uv run -m pytest --cov=samer_agent -vv tests/component --cov-report=lcov:coverage/component.info
